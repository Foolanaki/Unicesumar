...
{
	int codigoProjeto;
	char nomeProjeto[30], descricaoProjeto[200], anoProjeto[5], dataLimiteProjeto[20], setorDestinoProjeto[30], gerenteProjeto[30], statusProjeto[2], equipeProjeto[1];
}dados;
...
void cadastrarProjeto(dados cadastro[MAX])
{
	...
	while(resposta == 's' && posicao <= MAX)
	{
		...
		gets(cadastro[posicao].descricaoProjeto);
		fflush(stdin);
		...
		gets(cadastro[posicao].dataLimiteProjeto);
		fflush(stdin);
